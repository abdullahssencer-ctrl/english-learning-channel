...
import { practicalStoriesB1 } from '../data/practicalStoriesB1';
import { BookOpen, ChevronDown, ChevronUp, Lightbulb, FileText } from 'lucide-react';

export default function PracticalStoriesB1({ user }) {
      ...
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            B1 seviyesindeki en kritik dil bilgisi kurallarını cümle düzeyinde pekiştirin. Her cümlenin detaylı gramer analiziyle öğrenin.
          </p>
        </div>

        {!selectedStory ? (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {practicalStoriesB1.map((story) => (
              <div
                key={story.id}
                onClick={() => setSelectedStory(story)}
                className="bg-white rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 cursor-pointer border-2 border-transparent hover:border-purple-500 p-6"
              >
              ...
              <div className="grid md:grid-cols-2 gap-3">
                {selectedStory.vocabulary.map((vocab, index) => (
                  <div key={index} className="bg-white rounded-lg p-3 shadow-sm">
                    <span className="font-medium text-purple-600">{vocab.turkish}</span>
                  ...
                  >
                    ...
                    {expandedSentence === index && (
                      <div className="p-4 bg-white border-t border-gray-200">
                        <div className="mb-4">
                          <p className="text-sm text-purple-600 font-medium mb-1">İngilizce:</p>
                          <p className="text-gray-900 font-semibold">{sentence.english}</p>
                        </div>
                        <div>
                          <p className="text-sm text-purple-600 font-medium mb-1">Gramer Analizi:</p>
