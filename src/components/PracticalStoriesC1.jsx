...
import { practicalStoriesC1 } from '../data/practicalStoriesC1';
import { BookOpen, ChevronDown, ChevronUp, Lightbulb, FileText } from 'lucide-react';

export default function PracticalStoriesC1({ user }) {
      ...
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            C1 Advanced seviyesinde çeviri ve gelişmiş cümle kurma becerilerinizi geliştirin. Her cümlenin detaylı gramer analiziyle öğrenin.
          </p>
        </div>

        {!selectedStory ? (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {practicalStoriesC1.map((story) => (
              <div
                key={story.id}
                onClick={() => setSelectedStory(story)}
                className="bg-white rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 cursor-pointer border-2 border-transparent hover:border-red-500 p-6"
              >
          ...
          <div className="bg-white rounded-2xl shadow-2xl p-8">
            <button
              ...
              className="mb-6 text-red-600 hover:text-red-800 font-medium flex items-center gap-2"
            >
              ← Hikayelere Dön
            </button>
            ...
            <div className="mb-8">
              ...
              <span className="inline-block bg-red-100 text-red-700 text-sm px-4 py-2 rounded-full">
              ...
              <div className="grid md:grid-cols-2 gap-3">
                {selectedStory.vocabulary.map((vocab, index) => (
                  <div key={index} className="bg-white rounded-lg p-3 shadow-sm">
                    <span className="font-medium text-red-600">{vocab.turkish}</span>
                  ...
                  >
                    ...
                    {expandedSentence === index && (
                      <div className="p-4 bg-white border-t border-gray-200">
                        <div className="mb-4">
                          <p className="text-sm text-red-600 font-medium mb-1">İngilizce:</p>
                          <p className="text-gray-900 font-semibold">{sentence.english}</p>
                        </div>
                        <div>
                          <p className="text-sm text-red-600 font-medium mb-1">Gramer Analizi:</p>
